...
export default function QACallAssistant() {
  ...
  const doneCount = useMemo(
    () => visibleSections.flatMap((section) => section.items).filter((item) => checked[item.id]).length,
    [checked, visibleSections]
  );

  const totalCount = visibleSections.reduce((acc, section) => acc + section.items.length, 0);
        ...
        <div className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            ...
            <div className="rounded-2xl bg-slate-100 px-4 py-3 text-sm">
              ...
              <div className="text-slate-600">{totalCount - doneCount} left</div>
            </div>
          </div>
